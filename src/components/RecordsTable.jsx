                  ...
                  <div className="flex items-center justify-center gap-2 flex-wrap">
                    {r.sessionId ? (
                      <>
                        ...
                        {hasAISolution(r.sessionId) && (
                          <button
                            onClick={() =>
                              navigate(`/session/${r.sessionId}/solution`)
                            }
                            className="inline-flex items-center gap-1 px-3 py-1 bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white text-xs rounded-md transition-all shadow-sm hover:shadow-md"
                          >
                      ...
                      <button
                        onClick={async () => {
                          const ok = window.confirm(
                            "Hapus record ini? Tindakan ini mungkin menghapus data sesi terkait.",
                          );
                          if (!ok) return;
                          try {
                            const resp = await authFetch(
                              `${API_BASE_URL}/api/stress-history/${r.id}`,
                              { method: "DELETE" },
                            );
                            if (!resp.ok) {
                            ...
                            // Refresh via callback if provided
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1 bg-rose-500 hover:bg-rose-600 text-white text-xs rounded-md transition-colors"
                      >
                        Delete
                      </button>
                    ) : null}
